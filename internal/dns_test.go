...
func TestDns_VerifyFCrDNS(t *testing.T) {
	...
	tests := []struct {
		...
		{"valid", "8.8.8.8", true},
		{"valid partial", "1.1.1.1", true},
		{"invalid", "9.9.9.9", false},
		{"reverse lookup fails", "1.2.3.4", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := d.VerifyFCrDNS(tt.ip); got != tt.want {
	...
	t.Run("reverse cache hit", func(t *testing.T) {
		// Prime the cache
		if got := d.VerifyFCrDNS("8.8.8.8"); got != true {
...
func TestDns_VerifyFCrDNSWithPattern(t *testing.T) {
	d := NewDNS(1, 1)
	...
	tests := []struct {
		...
		{"valid match", "8.8.8.8", `\.google$`, true},
		{"valid no match", "8.8.8.8", `\.com$`, false},
		{"invalid", "9.9.9.9", ".*", false},
		{"invalid pattern", "8.8.8.8", `[`, false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := d.VerifyFCrDNSWithPattern(tt.ip, tt.pattern); got != tt.want {
				t.Errorf("VerifyFCrDNSWithPattern() = %v, want %v", got, tt.want)
			}
		})
	}
}
